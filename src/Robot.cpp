...
{
	glPushMatrix();
		glColor3f(0.2f, 0.7f, 1.0f);	// white
		glTranslatef(xPos, yPos, zPos);
...
void Robot::DrawLeg(float xPos, float yPos, float zPos)
{
	glPushMatrix();
		...
		glColor3f(0.5f, 0.5f, 0.8f);	// yellow
		DrawCube(0.0f, 0.0f, 0.0f);
	glPopMatrix();
}
...
void Robot::DrawRobot(float xPos, float yPos, float zPos, float rotation)
{
	glPushMatrix();	
		...
		glRotatef(rotation, 0.0f, 1.0f, 0.0f);	// rotate the robot on its y-axis
...
void Robot::Prepare(float dt)
{
	...
	// if leg is moving forward, increase angle, else decrease angle
		for (char side = 0; side < 2; side++)
		{
			...
			if (armAngles[side] >= 30.0f)
				armStates[side] = BACKWARD_STATE;
			else if (armAngles[side] <= -30.0f)
			...
			// change state if exceeding angles
			if (legAngles[side] >= 30.0f)
				legStates[side] = BACKWARD_STATE;
			else if (legAngles[side] <= -30.0f)
				legStates[side] = FORWARD_STATE;
		}
	}
}

void Robot::setMoving()
{
	isMoving = true;
}

void Robot::setNotMoving()
{
	isMoving = false;
	resetLimbsPosition();
}
...
int Robot::numOfKeysDown()
{
	return keysDown;
}

void Robot::keyPressed()
{
	keysDown += 1;
}

void Robot::keyReleased()
{
	keysDown -= 1;
}

void Robot::resetLimbsPosition() {
	for (char side = 0; side < 2; side++)
	{
		armAngles[side] = 0.f;
		legAngles[side] = 0.f;
